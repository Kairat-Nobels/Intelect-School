    ...
    <div className={styles.wrapper}>
      <div className='container'>
        <div className={styles.header}>
          <button className='adminBtn' onClick={() => navigate('/')}>Главная</button>
          <h2>Администратор</h2>
          <button className='adminBtn red' appearance="ghost" onClick={handleLogout}>Выйти</button>
        </div>
        <div className={styles.navbar}>
          <NavLink to="/admin/records" className={({ isActive }) => isActive ? styles.active : ''}>Заявки</NavLink>
          <NavLink to="/admin/reviews" className={({ isActive }) => isActive ? styles.active : ''}>Отзывы</NavLink>
          <NavLink to="/admin/services" className={({ isActive }) => isActive ? styles.active : ''}>Расписания</NavLink>
          <NavLink to="/admin/staff" className={({ isActive }) => isActive ? styles.active : ''}>Учителя</NavLink>
          <NavLink to="/admin/news" className={({ isActive }) => isActive ? styles.active : ''}>Новости</NavLink>
        </div>
